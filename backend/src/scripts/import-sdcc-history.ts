...
async function importSDCCHistory() {
  try {
    console.log('🚀 Démarrage de l\'import des données historiques SDCC...\n');

    // Chemin vers le fichier Excel
    const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'sdcc', 'sdcc.xlsx');
    ...
    // Convertir en JSON
    const rawData: SDCCRow[] = XLSX.utils.sheet_to_json(worksheet);
    ...
    // Vérifier si SDCC existe dans la base de données
    let stock = await prisma.stock.findUnique({
      where: { symbol: 'SDCC' }
    });

    // Si SDCC n'existe pas, le créer
    if (!stock) {
      console.log('📝 SDCC non trouvé dans la base de données, création...');
      stock = await prisma.stock.create({
        data: {
          symbol: 'SDCC',
          company_name: 'SODE CI',
      ...
      console.log('✅ SDCC créé avec succès\n');
    } else {
      console.log('✅ SDCC trouvé dans la base de données\n');
    }
    ...
    // Vérifier le nombre total de données historiques pour SDCC
    const totalHistory = await prisma.stockHistory.count({
      where: { stock_ticker: 'SDCC' }
    });

    console.log(`📊 Total des données historiques SDCC dans la base: ${totalHistory}\n`);
