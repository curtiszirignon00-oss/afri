...
async function importSCRCHistory() {
    try {
        console.log('🚀 Démarrage de l\'import des données historiques SCRC...\n');

        // Chemin vers le fichier Excel
        const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'scrc', 'scrc.xlsx');
        ...
        // Convertir en JSON
        const rawData: SCRCRow[] = XLSX.utils.sheet_to_json(worksheet);
        ...
        // Vérifier si SCRC existe dans la base de données
        let stock = await prisma.stock.findUnique({
            where: { symbol: 'SCRC' }
        });

        // Si SCRC n'existe pas, le créer
        if (!stock) {
            console.log('📝 SCRC non trouvé dans la base de données, création...');
            stock = await prisma.stock.create({
                data: {
                    symbol: 'SCRC',
                    company_name: 'SUCRIVOIRE CI',
            ...
            console.log('✅ SCRC créé avec succès\n');
        } else {
            console.log('✅ SCRC trouvé dans la base de données\n');
        }
        ...
        // Vérifier le nombre total de données historiques pour SCRC
        const totalHistory = await prisma.stockHistory.count({
            where: { stock_ticker: 'SCRC' }
        });

        console.log(`📊 Total des données historiques SCRC dans la base: ${totalHistory}\n`);
