...
async function importBOABHistory() {
    try {
        console.log('🚀 Démarrage de l\'import des données historiques BOAB...\n');

        // Chemin vers le fichier Excel
        const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'Boab', 'BOAB.xlsx');
        ...
        // Convertir en JSON
        const rawData: BOABRow[] = XLSX.utils.sheet_to_json(worksheet);
        ...
        // Vérifier si BOAB existe dans la base de données
        let stock = await prisma.stock.findUnique({
            where: { symbol: 'BOAB' }
        });

        // Si BOAB n'existe pas, le créer
        if (!stock) {
            console.log('📝 BOAB non trouvé dans la base de données, création...');
            stock = await prisma.stock.create({
                data: {
                    symbol: 'BOAB',
                    company_name: 'BANK OF AFRICA BENIN',
                    sector: 'Banques',
                    current_price: 0,
            ...
            console.log('✅ BOAB créé avec succès\n');
        } else {
            console.log('✅ BOAB trouvé dans la base de données\n');
        }
        ...
        // Vérifier le nombre total de données historiques pour BOAB
        const totalHistory = await prisma.stockHistory.count({
            where: { stock_ticker: 'BOAB' }
        });

        console.log(`📊 Total des données historiques BOAB dans la base: ${totalHistory}\n`);
