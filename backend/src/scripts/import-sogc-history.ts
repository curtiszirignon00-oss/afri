...
async function importSOGCHistory() {
    try {
        console.log('🚀 Démarrage de l\'import des données historiques SOGC...\n');

        // Chemin vers le fichier Excel
        const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'sogc', 'SOGC.xlsx');
        ...
        // Convertir en JSON
        const rawData: SOGCRow[] = XLSX.utils.sheet_to_json(worksheet);
        ...
        // Vérifier si SOGC existe dans la base de données
        let stock = await prisma.stock.findUnique({
            where: { symbol: 'SOGC' }
        });

        // Si SOGC n'existe pas, le créer
        if (!stock) {
            console.log('📝 SOGC non trouvé dans la base de données, création...');
            stock = await prisma.stock.create({
                data: {
                    symbol: 'SOGC',
                    company_name: 'SOGB CI',
            ...
            console.log('✅ SOGC créé avec succès\n');
        } else {
            console.log('✅ SOGC trouvé dans la base de données\n');
        }
        ...
        // Vérifier le nombre total de données historiques pour SOGC
        const totalHistory = await prisma.stockHistory.count({
            where: { stock_ticker: 'SOGC' }
        });

        console.log(`📊 Total des données historiques SOGC dans la base: ${totalHistory}\n`);
