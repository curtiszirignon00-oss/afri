...
async function importSIBCHistory() {
    try {
        console.log('🚀 Démarrage de l\'import des données historiques SIBC...\n');

        // Chemin vers le fichier Excel
        const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'sibc', 'sibc.xlsx');
        ...
        // Convertir en JSON
        const rawData: SIBCRow[] = XLSX.utils.sheet_to_json(worksheet);
        ...
        // Vérifier si SIBC existe dans la base de données
        let stock = await prisma.stock.findUnique({
            where: { symbol: 'SIBC' }
        });

        // Si SIBC n'existe pas, le créer
        if (!stock) {
            console.log('📝 SIBC non trouvé dans la base de données, création...');
            ...
            console.log('✅ SIBC créé avec succès\n');
        } else {
            console.log('✅ SIBC trouvé dans la base de données\n');
        }
        ...
        // Vérifier le nombre total de données historiques pour SIBC
        const totalHistory = await prisma.stockHistory.count({
            where: { stock_ticker: 'SIBC' }
        });

        console.log(`📊 Total des données historiques SIBC dans la base: ${totalHistory}\n`);
