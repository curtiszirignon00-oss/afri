...
async function importSIVCHistory() {
  try {
    console.log('🚀 Démarrage de l\'import des données historiques SIVC...\n');

    // Chemin vers le fichier Excel
    const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'sivc', 'sivc.xlsx');
    ...
    // Convertir en JSON
    const rawData: SIVCRow[] = XLSX.utils.sheet_to_json(worksheet);
    ...
    // Vérifier si SIVC existe dans la base de données
    let stock = await prisma.stock.findUnique({
      where: { symbol: 'SIVC' }
    });

    // Si SIVC n'existe pas, le créer
    if (!stock) {
      console.log('📝 SIVC non trouvé dans la base de données, création...');
      stock = await prisma.stock.create({
        data: {
          symbol: 'SIVC',
          company_name: 'Air Liquide Côte d\'Ivoire',
          sector: 'Industriels',
          current_price: 0,
      ...
      console.log('✅ SIVC créé avec succès\n');
    } else {
      console.log('✅ SIVC trouvé dans la base de données\n');
    }
    ...
    // Vérifier le nombre total de données historiques pour SIVC
    const totalHistory = await prisma.stockHistory.count({
      where: { stock_ticker: 'SIVC' }
    });

    console.log(`📊 Total des données historiques SIVC dans la base: ${totalHistory}\n`);
