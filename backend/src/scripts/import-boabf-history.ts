...
async function importBOABFHistory() {
    try {
        console.log('🚀 Démarrage de l\'import des données historiques BOABF...\n');

        // Chemin vers le fichier Excel
        const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'Boabf', 'BOABF.xlsx');
        ...
        // Convertir en JSON
        const rawData: BOABFRow[] = XLSX.utils.sheet_to_json(worksheet);
        ...
        // Vérifier si BOABF existe dans la base de données
        let stock = await prisma.stock.findUnique({
            where: { symbol: 'BOABF' }
        });

        // Si BOABF n'existe pas, le créer
        if (!stock) {
            console.log('📝 BOABF non trouvé dans la base de données, création...');
            stock = await prisma.stock.create({
                data: {
                    symbol: 'BOABF',
                    company_name: 'BANK OF AFRICA BURKINA FASO',
                    sector: 'Banques',
                    current_price: 0,
            ...
            console.log('✅ BOABF créé avec succès\n');
        } else {
            console.log('✅ BOABF trouvé dans la base de données\n');
        }
        ...
        // Vérifier le nombre total de données historiques pour BOABF
        const totalHistory = await prisma.stockHistory.count({
            where: { stock_ticker: 'BOABF' }
        });

        console.log(`📊 Total des données historiques BOABF dans la base: ${totalHistory}\n`);
