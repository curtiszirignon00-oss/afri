...
async function importSLBCHistory() {
    try {
        console.log('🚀 Démarrage de l\'import des données historiques SLBC...\n');

        // Chemin vers le fichier Excel
        const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'slbc', 'SLBC.xlsx');
        ...
        // Convertir en JSON
        const rawData: SLBCRow[] = XLSX.utils.sheet_to_json(worksheet);
        ...
        // Si SLBC n'existe pas, le créer
        if (!stock) {
            console.log('📝 SLBC non trouvé dans la base de données, création...');
            ...
            console.log('✅ SLBC créé avec succès\n');
        } else {
            console.log('✅ SLBC trouvé dans la base de données\n');
        }
        ...
        // Vérifier le nombre total de données historiques pour SLBC
        const totalHistory = await prisma.stockHistory.count({
            where: { stock_ticker: 'SLBC' }
        });

        console.log(`📊 Total des données historiques SLBC dans la base: ${totalHistory}\n`);
