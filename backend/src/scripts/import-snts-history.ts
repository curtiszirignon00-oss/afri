...
async function importSNTSHistory() {
    try {
        console.log('🚀 Démarrage de l\'import des données historiques SNTS...\n');

        // Chemin vers le fichier Excel
        const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'snts', 'SNTS.xlsx');
        ...
        // Convertir en JSON
        const rawData: SNTSRow[] = XLSX.utils.sheet_to_json(worksheet);
        ...
        // Vérifier si SNTS existe dans la base de données
        let stock = await prisma.stock.findUnique({
            where: { symbol: 'SNTS' }
        });

        // Si SNTS n'existe pas, le créer
        if (!stock) {
            console.log('📝 SNTS non trouvé dans la base de données, création...');
            stock = await prisma.stock.create({
                data: {
                    symbol: 'SNTS',
                    company_name: 'SONATEL SN',
            ...
            console.log('✅ SNTS créé avec succès\n');
        } else {
            console.log('✅ SNTS trouvé dans la base de données\n');
        }
        ...
        // Vérifier le nombre total de données historiques pour SNTS
        const totalHistory = await prisma.stockHistory.count({
            where: { stock_ticker: 'SNTS' }
        });

        console.log(`📊 Total des données historiques SNTS dans la base: ${totalHistory}\n`);
