...
async function verifyFTSCData() {
  try {
    console.log('🔍 Vérification complète des données FTSC...\n');

    const ticker = 'FTSC';

    // 1. Stock
    console.log('📊 Stock FTSC:');
    ...
    console.log('📈 Données historiques FTSC:');
    ...
    // 3. Fondamentaux
    console.log('💰 Données fondamentales FTSC:');
    ...
    console.log('🏢 Informations compagnie FTSC:');
    ...
    console.log('📅 Données financières annuelles FTSC:');
    ...
    if (annualFinancials.length > 0) {
      ...
      console.log('  Année │ CA (MFCFA)    │ Croissance CA │ RN (MFCFA)    │ Croissance RN │ BPA      │ PER      │ Dividende');
      console.log('  ' + '─'.repeat(100));
