...
async function importTTLCHistory() {
  try {
    console.log('🚀 Démarrage de l\'import des données historiques TTLC...\n');

    // Chemin vers le fichier Excel
    const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'ttlc', 'TTLC.xlsx');
    ...
    // Convertir en JSON
    const rawData: TTLCRow[] = XLSX.utils.sheet_to_json(worksheet);
    ...
    // Vérifier si TTLC existe dans la base de données
    let stock = await prisma.stock.findUnique({
      where: { symbol: 'TTLC' }
    });

    // Si TTLC n'existe pas, le créer
    if (!stock) {
      console.log('📝 TTLC non trouvé dans la base de données, création...');
      stock = await prisma.stock.create({
        data: {
          symbol: 'TTLC',
          company_name: 'TOTAL CI LOGISTIQUE',
          sector: 'Industrie',
          current_price: 0,
      ...
      console.log('✅ TTLC créé avec succès\n');
    } else {
      console.log('✅ TTLC trouvé dans la base de données\n');
    }
    ...
    // Vérifier le nombre total de données historiques pour TTLC
    const totalHistory = await prisma.stockHistory.count({
      where: { stock_ticker: 'TTLC' }
    });

    console.log(`📊 Total des données historiques TTLC dans la base: ${totalHistory}\n`);
