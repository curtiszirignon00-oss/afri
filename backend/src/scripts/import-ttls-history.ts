...
async function importTTLSHistory() {
  try {
    console.log('🚀 Démarrage de l\'import des données historiques TTLS...\n');

    // Chemin vers le fichier Excel
    const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'ttls', 'TTLS.xlsx');
    ...
    // Convertir en JSON
    const rawData: TTLSRow[] = XLSX.utils.sheet_to_json(worksheet);
    ...
    // Vérifier si TTLS existe dans la base de données
    let stock = await prisma.stock.findUnique({
      where: { symbol: 'TTLS' }
    });

    // Si TTLS n'existe pas, le créer
    if (!stock) {
      console.log('📝 TTLS non trouvé dans la base de données, création...');
      stock = await prisma.stock.create({
        data: {
          symbol: 'TTLS',
          company_name: 'TOTAL CI',
          sector: 'Energie',
          current_price: 0,
      ...
      console.log('✅ TTLS créé avec succès\n');
    } else {
      console.log('✅ TTLS trouvé dans la base de données\n');
    }
    ...
    // Vérifier le nombre total de données historiques pour TTLS
    const totalHistory = await prisma.stockHistory.count({
      where: { stock_ticker: 'TTLS' }
    });

    console.log(`📊 Total des données historiques TTLS dans la base: ${totalHistory}\n`);
