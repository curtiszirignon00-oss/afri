...
async function importUNLCHistory() {
  try {
    console.log('🚀 Démarrage de l\'import des données historiques UNLC...\n');

    // Chemin vers le fichier Excel
    const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'unlc', 'UNLC.xlsx');
    ...
    // Convertir en JSON
    const rawData: UNLCRow[] = XLSX.utils.sheet_to_json(worksheet);
    ...
    // Vérifier si UNLC existe dans la base de données
    let stock = await prisma.stock.findUnique({
      where: { symbol: 'UNLC' }
    });

    // Si UNLC n'existe pas, le créer
    if (!stock) {
      console.log('📝 UNLC non trouvé dans la base de données, création...');
      stock = await prisma.stock.create({
        data: {
          symbol: 'UNLC',
          company_name: 'UNILEVER CI',
      ...
      console.log('✅ UNLC créé avec succès\n');
    } else {
      console.log('✅ UNLC trouvé dans la base de données\n');
    }
    ...
    // Vérifier le nombre total de données historiques pour UNLC
    const totalHistory = await prisma.stockHistory.count({
      where: { stock_ticker: 'UNLC' }
    });

    console.log(`📊 Total des données historiques UNLC dans la base: ${totalHistory}\n`);
