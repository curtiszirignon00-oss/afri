...
async function importBICBHistory() {
    try {
        console.log('🚀 Démarrage de l\'import des données historiques BICB...\n');

        // Chemin vers le fichier Excel
        const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'bicb', 'bicb.xlsx');
        ...
        // Vérifier si BICB existe dans la base de données
        let stock = await prisma.stock.findUnique({
            where: { symbol: 'BICB' }
        });

        // Si BICB n'existe pas, le créer
        if (!stock) {
            console.log('📝 BICB non trouvé dans la base de données, création...');
            stock = await prisma.stock.create({
                data: {
                    symbol: 'BICB',
                    company_name: 'BIC-BRED BANQUE POPULAIRE CI',
                    sector: 'Finance',
                    current_price: 0,
            ...
            console.log('✅ BICB créé avec succès\n');
        } else {
            console.log('✅ BICB trouvé dans la base de données\n');
        }
        ...
        for (let i = 0; i < rawData.length; i++) {
            ...
            try {
                ...
                let openValue = row.Open || row.open || row.OPEN || row.Ouverture || row['Ouv.'];
                let highValue = row.High || row.high || row.HIGH || row['Plus haut'] || row[' Plus Haut'] || row['Plus Haut'];
                let lowValue = row.Low || row.low || row.LOW || row['Plus bas'] || row['Plus Bas'];
                let closeValue = row.Close || row.close || row.CLOSE || row['Clôture'] || row.Cloture || row.Dernier;
                let volumeValue = row.Volume || row.volume || row.VOLUME || row['Volume Titres'] || row['Vol.'] || row['Volume FCFA'];
        ...
        // Vérifier le nombre total de données historiques pour BICB
        const totalHistory = await prisma.stockHistory.count({
            where: { stock_ticker: 'BICB' }
        });

        console.log(`📊 Total des données historiques BICB dans la base: ${totalHistory}\n`);
