...
async function importPRSCHistory() {
    try {
        console.log('🚀 Démarrage de l\'import des données historiques PRSC...\n');

        // Chemin vers le fichier Excel
        const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'prsc', 'prsc.xlsx');
        ...
        // Convertir en JSON
        const rawData: PRSCRow[] = XLSX.utils.sheet_to_json(worksheet);
        ...
        // Vérifier si PRSC existe dans la base de données
        let stock = await prisma.stock.findUnique({
            where: { symbol: 'PRSC' }
        });

        // Si PRSC n'existe pas, le créer
        if (!stock) {
            console.log('📝 PRSC non trouvé dans la base de données, création...');
            stock = await prisma.stock.create({
                data: {
                    symbol: 'PRSC',
                    company_name: 'TRACTAFRIC MOTORS CI',
            ...
            console.log('✅ PRSC créé avec succès\n');
        } else {
            console.log('✅ PRSC trouvé dans la base de données\n');
        }
        ...
        // Vérifier le nombre total de données historiques pour PRSC
        const totalHistory = await prisma.stockHistory.count({
            where: { stock_ticker: 'PRSC' }
        });

        console.log(`📊 Total des données historiques PRSC dans la base: ${totalHistory}\n`);
