...
async function importSHECHistory() {
    try {
        console.log('🚀 Démarrage de l\'import des données historiques SHEC...\n');

        // Chemin vers le fichier Excel
        const excelPath = path.join('C:', 'Users', 'HP', 'OneDrive', 'Desktop', 'actions brvm', 'shec', 'shec.xlsx');
        ...
        // Convertir en JSON
        const rawData: SHECRow[] = XLSX.utils.sheet_to_json(worksheet);
        ...
        // Vérifier si SHEC existe dans la base de données
        let stock = await prisma.stock.findUnique({
            where: { symbol: 'SHEC' }
        });

        // Si SHEC n'existe pas, le créer
        if (!stock) {
            console.log('📝 SHEC non trouvé dans la base de données, création...');
            stock = await prisma.stock.create({
                data: {
                    symbol: 'SHEC',
                    company_name: 'SHELL CI',
                    sector: 'Energie',
                    current_price: 0,
            ...
            console.log('✅ SHEC créé avec succès\n');
        } else {
            console.log('✅ SHEC trouvé dans la base de données\n');
        }
        ...
        // Vérifier le nombre total de données historiques pour SHEC
        const totalHistory = await prisma.stockHistory.count({
            where: { stock_ticker: 'SHEC' }
        });

        console.log(`📊 Total des données historiques SHEC dans la base: ${totalHistory}\n`);
