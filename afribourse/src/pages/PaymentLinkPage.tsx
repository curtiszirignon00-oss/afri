...
export default function PaymentLinkPage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/payment-links/${token}`)
      .then(r => r.json())
      .then(data => {
  ...
  if (step === 'success') return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-green-50 to-emerald-50">
      <div className="text-center max-w-sm bg-white rounded-2xl shadow-xl p-8">
        ...
        <p className="text-gray-500 text-sm">Entrez votre PIN Mobile Money pour finaliser.</p>
        <div className="mt-6 bg-green-50 rounded-xl p-4">
          <p className="text-sm font-semibold text-green-800">{link?.planName}</p>
          <p className="text-2xl font-extrabold text-green-700">{formatPrice(link?.amount ?? 0)}</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-700 to-indigo-800 px-6 py-6 text-white">
          <p className="text-blue-200 text-xs font-bold uppercase tracking-widest mb-1">Lien de paiement sécurisé</p>
          <h1 className="text-2xl font-extrabold leading-snug">{link?.planName}</h1>
          {link?.firstName && <p className="text-blue-200 text-sm mt-1">Pour {link.firstName}</p>}
          <div className="mt-3 inline-block bg-white/15 rounded-xl px-4 py-2">
            <p className="text-3xl font-extrabold">{formatPrice(link?.amount ?? 0)}</p>
          </div>
          {link?.note && (
            <p className="text-blue-200 text-xs mt-2 italic">{link.note}</p>
          )}
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          >
            {step === 'paying' ? (
              ...
              `Payer ${formatPrice(link?.amount ?? 0)}`
            )}
          </button>
