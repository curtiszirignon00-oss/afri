...
import { User, Settings, Wallet, PlusCircle, ShoppingCart, TrendingUp, TrendingDown, X, Eye, LineChart as ChartIcon, AlertCircle } from 'lucide-react';
...
export default function DashboardPage({ onNavigate }: DashboardPageProps) {
  // ✅ Hook personnalisé pour le portfolio
  const { 
    portfolio, 
    stocksData, 
    portfolioHistory, 
  ...
  useEffect(() => {
    loadUserData();
  }, [stocksData]); // Recharger quand stocksData change
  ...
  async function loadUserData() {
    ...
    try {
      ...
      // Filtrer les actions de la watchlist depuis stocksData
  ...
  async function handleSell() {
    ...
    setIsSelling(true);
    const toastId = toast.loading('Vente...');
    setError(null);
    
    try {
      const response = await fetch(`${API_BASE_URL}/portfolios/my/sell`, {
        method: 'POST',
      ...
      toast.success('Vente réussie !', { id: toastId });
      setSellModalOpen(false);
      await reloadPortfolio(); // ✅ Utilise le hook
      await loadUserData();
    } catch (err: any) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh] text-center px-4">
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
          ...
          <button onClick={handleLogout} className="ml-4 px-2 py-1 bg-red-600 text-white text-xs rounded align-middle">
            Déconnexion
          </button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center space-x-4">
            <button onClick={() => onNavigate('profile')} className="flex items-center space-x-2 px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
          ...
          <div className="lg:col-span-2 space-y-8">
            {portfolio ? (
              <>
                    ...
                    <div style={{ width: '100%', height: 300 }}>
                      <ResponsiveContainer>
                        <LineChart data={portfolioHistory} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                          ...
                          <YAxis width={80} fontSize={12} tick={{ fill: '#6b7280' }} tickFormatter={(v) => new Intl.NumberFormat('fr-FR', { notation: 'compact' }).format(v as number)} />
                          <Tooltip contentStyle={{ borderRadius: '0.75rem', borderColor: '#e5e7eb' }} formatter={(v) => [`${formatNumber(v as number)} FCFA`, 'Valeur']} />
                          <Legend wrapperStyle={{ fontSize: "14px" }} />
                          <Line type="monotone" dataKey="value" name="Valeur du portefeuille" stroke="#4f46e5" strokeWidth={2} dot={false} />
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                  ) : (
                    <p className="text-gray-500 text-center py-10">Données historique non disponibles.</p>
                  )}
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => onNavigate('transactions')} className="text-sm font-semibold text-blue-600 hover:text-blue-800">
                      Voir historique
                    </button>
                  </div>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="bg-blue-50 rounded-lg p-4">
                      <p className="text-sm text-gray-600 mb-2">Valeur Totale Estimée</p>
                      <p className="text-3xl font-bold">{formatNumber(totalPortfolioValue)} FCFA</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-4">
                      <p className="text-sm text-gray-600 mb-2">Liquidités</p>
                      <p className="text-3xl font-bold">{formatNumber(portfolio.cash_balance)} FCFA</p>
                    </div>
                  </div>
                </div>
                ...
                {/* Positions Table */}
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  <h3 className="text-xl font-bold text-gray-900 mb-4">Mes Positions</h3>
                  {positions.length > 0 ? (
                      ...
                      <table className="w-full text-sm text-left">
                        <thead className="text-xs text-gray-500 uppercase bg-gray-50">
                          <tr>
                            ...
                            <th className="px-4 py-3 text-right">P/L Estimé</th>
                            <th className="px-4 py-3 text-center">Action</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200">
                          {positions.map(p => {
                            const stockMarketData = stocksData[p.stock_ticker];
                            if (!stockMarketData) {
                              return (
                                <tr key={p.id} className="opacity-50">
                                  <td className="px-4 py-4 font-bold">{p.stock_ticker}</td>
                                  <td colSpan={5} className="px-4 py-4 text-center text-gray-500">
                                    Données marché non chargées...
                                  </td>
                                </tr>
                              );
                            }

                            const currentVal = p.quantity * stockMarketData.current_price;
                            ...
                            return (
                              <tr key={p.id}>
                                <td className="px-4 py-4">
                                  <div className="font-bold">{stockMarketData.symbol}</div>
                                  <div className="text-gray-600 text-xs">{stockMarketData.company_name}</div>
                                </td>
                                <td className="px-4 py-4 text-right">{formatNumber(p.quantity)}</td>
                                <td className="px-4 py-4 text-right">{formatNumber(stockMarketData.current_price)}</td>
                                <td className="px-4 py-4 text-right font-medium">{formatNumber(currentVal)}</td>
                                <td className="px-4 py-4 text-right">
                                  <div className={pnl >= 0 ? 'text-green-600' : 'text-red-600'}>
                                    {pnl >= 0 ? '+' : ''}{formatNumber(pnl)} FCFA
                                  </div>
                    ...
                    <div className="text-center py-8">
                      ...
                      <h4 className="font-semibold">Aucune action détenue</h4>
                      <p className="text-gray-500 mt-2">Explorez les marchés pour acheter.</p>
                      <button onClick={() => onNavigate('markets')} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">
                        Explorer Marchés
                      </button>
                    </div>
                  )}
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center flex flex-col items-center">
                ...
                <h2 className="text-xl font-semibold mb-2">Commencez simulation</h2>
                <p className="text-gray-600 mb-6 max-w-sm">Créez un portefeuille virtuel gratuit.</p>
                <button onClick={createPortfolio} disabled={portfolioLoading} className="flex items-center space-x-2 px-6 py-3 bg-blue-600 text-white rounded-lg disabled:opacity-50 hover:bg-blue-700">
                  {portfolioLoading ? (
                    <div className="animate-spin rounded-full h-5 w-5 border-t-2 border-white"></div>
                  ) : (
                    <PlusCircle className="w-5 h-5" />
                  )}
                  <span>{portfolioLoading ? 'Création...' : 'Créer portefeuille'}</span>
                </button>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setSellModalOpen(false)} className="p-1 rounded-full text-gray-400 hover:bg-gray-100 hover:text-gray-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setSellQuantity(Math.max(1, Math.min(v, selectedPosition.quantity)));
                }}
                min="1"
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 space-y-2 text-sm">
              <div className="flex justify-between">
                <span>Prix actuel / action</span>
                <span className="font-medium">{formatNumber(stocksData[selectedPosition.stock_ticker].current_price)} FCFA</span>
              </div>
              <div className="flex justify-between font-bold text-base">
                <span>Revenu estimé</span>
                <span className="text-green-600">{formatNumber(sellQuantity * stocksData[selectedPosition.stock_ticker].current_price)} FCFA</span>
              </div>
            </div>
            <div className="mt-6 flex gap-4">
              <button onClick={() => setSellModalOpen(false)} className="flex-1 px-4 py-3 bg-gray-200 text-gray-800 rounded-lg font-semibold hover:bg-gray-300 transition-colors">
                Annuler
              </button>
              <button onClick={handleSell} disabled={isSelling} className="flex-1 px-4 py-3 bg-red-600 text-white rounded-lg font-semibold disabled:opacity-50 hover:bg-red-700 transition-colors">
                {isSelling ? (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setBuyModalOpen(false)} className="p-1 rounded-full text-gray-400 hover:bg-gray-100 hover:text-gray-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 space-y-2 text-sm">
              <div className="flex justify-between">
                <span>Prix actuel / action</span>
                <span className="font-medium">{formatNumber(selectedStockToBuy.current_price)} FCFA</span>
              </div>
              <div className="flex justify-between">
                <span>Liquidités disponibles</span>
                <span className="font-medium">{formatNumber(portfolio.cash_balance)} FCFA</span>
              </div>
              <div className="flex justify-between font-bold text-base">
                <span>Coût total estimé</span>
                <span className="text-red-600">{formatNumber(buyQuantity * selectedStockToBuy.current_price)} FCFA</span>
              </div>
              {(buyQuantity * selectedStockToBuy.current_price > portfolio.cash_balance) && (
                <p className="text-red-600 text-xs text-center pt-2">Fonds insuffisants.</p>
              )}
            </div>
            <div className="mt-6 flex gap-4">
              <button onClick={() => setBuyModalOpen(false)} className="flex-1 px-4 py-3 bg-gray-200 text-gray-800 rounded-lg font-semibold hover:bg-gray-300 transition-colors">
                Annuler
              </button>
              <button
                onClick={handleBuy}
                disabled={isBuying || (buyQuantity * selectedStockToBuy.current_price > portfolio.cash_balance)}
                className="flex-1 px-4 py-3 bg-green-600 text-white rounded-lg font-semibold disabled:opacity-50 hover:bg-green-700 transition-colors"
              >
                {isBuying ? (
                  <div className="animate-spin rounded-full h-5 w-5 border-t-2 border-white mx-auto"></div>
                ) : (
